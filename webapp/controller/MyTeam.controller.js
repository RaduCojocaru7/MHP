sap.ui.define([
  ...
  return Controller.extend("fbtool.controller.MyTeam", {
    ...
    _resolveManagerInfo: function () {
      ...
      if (oLogged) {
        // Extrage ID și nume din modelul loggedUser
        var id   = oLogged.getProperty("/userId") || "";
        var name = oLogged.getProperty("/fullName") || "";
        
        if (id || name) {
          try { if (id) localStorage.setItem("loggedUserId", id); } catch (e) {}
          return Promise.resolve({ id: String(id), name: String(name) });
        }
      }

      // Fallback: caută după email în UserSet
      var email = oLogged && oLogged.getProperty("/email") || "";
      ...
      return new Promise(function (resolve) {
        oModel.read("/UserSet", {
          ...
          success: function (oData) {
            ...
            var result = { 
              id: r && r.USER_ID ? String(r.USER_ID) : "", 
    ...
    _applyPmReqFilters: function () {
      ...
      if (!this._mgr || !this._mgr.id) {
        // Fallback: dacă nu avem ID, încearcă să-l găsești în lookup după nume
        if (this._mgr.name && this._userById) {
          var foundId = "";
          for (var key in this._userById) {
            if (this._userById[key] === this._mgr.name) {
              foundId = key;
              break;
            }
          }
          if (foundId) {
            this._mgr.id = foundId;
          } else {
            return;
          }
        } else {
      ...
      var managerId = this._normId(this._mgr.id);
